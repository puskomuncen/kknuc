...
<form name="fpendaftaranedit" id="fpendaftaranedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script<?= Nonce() ?>>
var currentTable = <?= json_encode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { pendaftaran: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fpendaftaranedit;
...
<?php if ($Page->id_pendaftaran->Visible) { // id_pendaftaran ?>
    <div id="r_id_pendaftaran"<?= $Page->id_pendaftaran->rowAttributes() ?>>
        <label id="elh_pendaftaran_id_pendaftaran" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_pendaftaran->caption() ?><?= $Page->id_pendaftaran->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_pendaftaran->getDisplayValue($Page->id_pendaftaran->getEditValue()))) ?>"></span>
<input type="hidden" data-table="pendaftaran" data-field="x_id_pendaftaran" data-hidden="1" name="x_id_pendaftaran" id="x_id_pendaftaran" value="<?= HtmlEncode($Page->id_pendaftaran->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script<?= Nonce() ?>>
loadjs.ready(["fpendaftaranedit", "datetimepicker"], function () {
...
loadjs.ready(['fpendaftaranedit', 'jqueryinputmask'], function() {
	...
	ew.createjQueryInputMask("fpendaftaranedit", "x_tanggal_daftar", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fpendaftaranedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fpendaftaranedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
<script>
loadjs.ready("head", function() { $("#fpendaftaranedit:first *:input[type!=hidden]:first").focus(),$("input").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("select").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("radio").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()})});
</script>
<?php } ?>
<script<?= Nonce() ?>>
