 ...
 * Page class
 */
class LaporanAkhirList extends LaporanAkhir
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "LaporanAkhirList";
    ...
    public string $FormName = "flaporan_akhirlist";

    // CSS class/style
    public string $CurrentPageName = "laporanakhirlist";
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Initialize URLs
        $this->AddUrl = "laporanakhiradd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "laporanakhirdelete";
        $this->MultiUpdateUrl = "laporanakhirupdate";
...
// Is modal
        ...
        if ($this->IsModal) {
            $this->FormName = "flaporan_akhirgrid";
        }
		...
		if (ReadCookie('laporan_akhir_searchpanel') == 'notactive' || ReadCookie('laporan_akhir_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('laporan_akhir_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("flaporan_akhirsrch");
        }
        $filterList = Concat($filterList, $this->id_laporan->AdvancedSearch->toJson(), ","); // Field id_laporan
        $filterList = Concat($filterList, $this->id_penempatan->AdvancedSearch->toJson(), ","); // Field id_penempatan
        $filterList = Concat($filterList, $this->file_laporan->AdvancedSearch->toJson(), ","); // Field file_laporan
        $filterList = Concat($filterList, $this->nilai_dosen->AdvancedSearch->toJson(), ","); // Field nilai_dosen
        $filterList = Concat($filterList, $this->nilai_instansi->AdvancedSearch->toJson(), ","); // Field nilai_instansi
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("flaporan_akhirsrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field id_laporan
        $this->id_laporan->AdvancedSearch->SearchValue = $filter["x_id_laporan"] ?? "";
        $this->id_laporan->AdvancedSearch->SearchOperator = $filter["z_id_laporan"] ?? "";
        $this->id_laporan->AdvancedSearch->SearchCondition = $filter["v_id_laporan"] ?? "";
        $this->id_laporan->AdvancedSearch->SearchValue2 = $filter["y_id_laporan"] ?? "";
        $this->id_laporan->AdvancedSearch->SearchOperator2 = $filter["w_id_laporan"] ?? "";
        $this->id_laporan->AdvancedSearch->save();
        ...
        // Field file_laporan
        $this->file_laporan->AdvancedSearch->SearchValue = $filter["x_file_laporan"] ?? "";
        $this->file_laporan->AdvancedSearch->SearchOperator = $filter["z_file_laporan"] ?? "";
        $this->file_laporan->AdvancedSearch->SearchCondition = $filter["v_file_laporan"] ?? "";
        $this->file_laporan->AdvancedSearch->SearchValue2 = $filter["y_file_laporan"] ?? "";
        $this->file_laporan->AdvancedSearch->SearchOperator2 = $filter["w_file_laporan"] ?? "";
        $this->file_laporan->AdvancedSearch->save();

        // Field nilai_dosen
        $this->nilai_dosen->AdvancedSearch->SearchValue = $filter["x_nilai_dosen"] ?? "";
        $this->nilai_dosen->AdvancedSearch->SearchOperator = $filter["z_nilai_dosen"] ?? "";
        $this->nilai_dosen->AdvancedSearch->SearchCondition = $filter["v_nilai_dosen"] ?? "";
        $this->nilai_dosen->AdvancedSearch->SearchValue2 = $filter["y_nilai_dosen"] ?? "";
        $this->nilai_dosen->AdvancedSearch->SearchOperator2 = $filter["w_nilai_dosen"] ?? "";
        $this->nilai_dosen->AdvancedSearch->save();

        // Field nilai_instansi
        $this->nilai_instansi->AdvancedSearch->SearchValue = $filter["x_nilai_instansi"] ?? "";
        $this->nilai_instansi->AdvancedSearch->SearchOperator = $filter["z_nilai_instansi"] ?? "";
        $this->nilai_instansi->AdvancedSearch->SearchCondition = $filter["v_nilai_instansi"] ?? "";
        $this->nilai_instansi->AdvancedSearch->SearchValue2 = $filter["y_nilai_instansi"] ?? "";
        $this->nilai_instansi->AdvancedSearch->SearchOperator2 = $filter["w_nilai_instansi"] ?? "";
        $this->nilai_instansi->AdvancedSearch->save();
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->id_laporan); // id_laporan
            $this->updateSort($this->id_penempatan); // id_penempatan
            $this->updateSort($this->file_laporan); // file_laporan
            $this->updateSort($this->nilai_dosen); // nilai_dosen
            $this->updateSort($this->nilai_instansi); // nilai_instansi
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->id_laporan->setSort("");
                $this->id_penempatan->setSort("");
                $this->file_laporan->setSort("");
                $this->nilai_dosen->setSort("");
                $this->nilai_instansi->setSort("");
            }
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"laporan_akhir\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"laporan_akhir\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"laporan_akhir\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"flaporan_akhirlist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"flaporan_akhirlist\" data-key=\"" . $this->keyToJson(true) .
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->id_laporan->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"laporan_akhir\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "id_laporan");
            $this->createColumnOption($option, "id_penempatan");
            $this->createColumnOption($option, "file_laporan");
            $this->createColumnOption($option, "nilai_dosen");
            $this->createColumnOption($option, "nilai_instansi");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"flaporan_akhirsrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"flaporan_akhirsrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="flaporan_akhirlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_laporan_akhir", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_laporan_akhir",
...
// Load basic search values
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->nilai_dosen->TooltipValue = "";

            // nilai_instansi
            $this->nilai_instansi->HrefValue = "";
            $this->nilai_instansi->TooltipValue = "";
        }
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('laporan_akhir_searchpanel') == 'notactive' || ReadCookie('laporan_akhir_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"flaporan_akhirsrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('laporan_akhir_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"flaporan_akhirsrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"flaporan_akhirsrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
