...
/**
 * Table class for logbook
 */
class Logbook extends DbTable implements LookupTableInterface
{
    ...
    // Fields
    public DbField $id_logbook;
    public DbField $id_penempatan;
    public DbField $tanggal;
    public DbField $kegiatan;
    public DbField $validasi_dosen;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "logbook"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id_logbook
        $this->id_logbook = new DbField(
            $this, // Table
            'x_id_logbook', // Variable name
            'id_logbook', // Name
            '`id_logbook`', // Expression
            '`id_logbook`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`id_logbook`', // Virtual expression
            false, // Is virtual
        ...
        $this->id_logbook->InputTextType = "text";
        $this->id_logbook->Raw = true;
        $this->id_logbook->IsAutoIncrement = true; // Autoincrement field
        $this->id_logbook->IsPrimaryKey = true; // Primary key field
        $this->id_logbook->Nullable = false; // NOT NULL field
        $this->id_logbook->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->id_logbook->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['id_logbook'] = &$this->id_logbook;
        ...
        $this->id_penempatan->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        ...
        // kegiatan
        $this->kegiatan = new DbField(
            $this, // Table
            'x_kegiatan', // Variable name
            'kegiatan', // Name
            '`kegiatan`', // Expression
            '`kegiatan`', // Basic search expression
            200, // Type
            65535, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`kegiatan`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->kegiatan->InputTextType = "text";
        $this->kegiatan->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['kegiatan'] = &$this->kegiatan;

        // validasi_dosen
        $this->validasi_dosen = new DbField(
            $this, // Table
            'x_validasi_dosen', // Variable name
            'validasi_dosen', // Name
            '`validasi_dosen`', // Expression
            '`validasi_dosen`', // Basic search expression
            16, // Type
            1, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`validasi_dosen`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->validasi_dosen->addMethod("getDefault", fn() => 0);
        $this->validasi_dosen->InputTextType = "text";
        $this->validasi_dosen->Raw = true;
        $this->validasi_dosen->setDataType(DataType::BOOLEAN);
        ...
        $this->validasi_dosen->DefaultErrorMessage = $this->language->phrase("IncorrectValueRegExp");
        $this->validasi_dosen->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        $this->Fields['validasi_dosen'] = &$this->validasi_dosen;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "logbook";
    }
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            $this->id_logbook->setDbValue($conn->lastInsertId());
            $row['id_logbook'] = $this->id_logbook->DbValue;
        }
        return $result;
    }
    ...
    public function update(array $row, string|array $where = "", ?array $old = null, bool $currentFilter = true): int|bool
    {
        ...
        if ($success) {
            if (!isset($row['id_logbook']) && !IsEmpty($this->id_logbook->CurrentValue)) {
                $row['id_logbook'] = $this->id_logbook->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('id_logbook', $row)) {
                AddFilter($where, QuotedName('id_logbook', $this->Dbid) . '=' . QuotedValue($row['id_logbook'], $this->id_logbook->DataType, $this->Dbid));
            }
        }
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->id_logbook->DbValue = $row['id_logbook'];
        $this->id_penempatan->DbValue = $row['id_penempatan'];
        $this->tanggal->DbValue = $row['tanggal'];
        $this->kegiatan->DbValue = $row['kegiatan'];
        $this->validasi_dosen->DbValue = $row['validasi_dosen'];
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->id_logbook->CurrentValue : $this->id_logbook->OldValue;
        if (IsEmpty($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey(string $key, bool $current = false, ?string $keySeparator = null): void
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->id_logbook->CurrentValue = $keys[0];
            } else {
                $this->id_logbook->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_logbook', $row) ? $row['id_logbook'] : null;
        } else {
            $val = !IsEmpty($this->id_logbook->OldValue) && !$current ? $this->id_logbook->OldValue : $this->id_logbook->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_logbook@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("logbooklist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "logbookview" => $this->language->phrase("View"),
            "logbookedit" => $this->language->phrase("Edit"),
            "logbookadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "LogbookView",
            Config("API_ADD_ACTION") => "LogbookAdd",
            Config("API_EDIT_ACTION") => "LogbookEdit",
            Config("API_DELETE_ACTION") => "LogbookDelete",
            Config("API_LIST_ACTION") => "LogbookList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("logbookview", $parm);
        } else {
            $url = $this->keyUrl("logbookview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("logbookedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("logbooklist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("logbookadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("logbooklist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("logbookdelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"id_logbook\":" . VarToJson($this->id_logbook->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_logbook->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_logbook->CurrentValue);
        } else {
    ...
    public function getRecordKeys(): array
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("id_logbook") ?? Route("id_logbook")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys(bool $setCurrent = true): string
    {
        ...
        foreach ($arKeys as $key) {
            if ($setCurrent) {
                $this->id_logbook->CurrentValue = $key;
            } else {
                $this->id_logbook->OldValue = $key;
            }
