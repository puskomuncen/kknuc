...
class PenempatanList extends Penempatan
{
    ...
    // Page object name
    public string $PageObjName = "PenempatanList";
    ...
    public string $FormName = "fpenempatanlist";

    // CSS class/style
    public string $CurrentPageName = "penempatanlist";
    ...
    public function setVisibility(): void
    {
        ...
        $this->id_pendaftaran->setVisibility();
        $this->id_instansi->setVisibility();
        $this->dosen_pembimbing->setVisibility();
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        if (!isset($GLOBALS["penempatan"]) || $GLOBALS["penempatan"]::class == PROJECT_NAMESPACE . "penempatan") {
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "penempatanview"); // If View page, no primary button
                } else { // List page
    ...
    protected function hideFieldsForAddEdit(): void
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->id_penempatan->Visible = false;
        }
    }
...
// Is modal
		...
		if (ReadCookie('penempatan_searchpanel') == 'notactive' || ReadCookie('penempatan_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('penempatan_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fpenempatansrch");
        }
        $filterList = Concat($filterList, $this->id_penempatan->AdvancedSearch->toJson(), ","); // Field id_penempatan
        $filterList = Concat($filterList, $this->id_pendaftaran->AdvancedSearch->toJson(), ","); // Field id_pendaftaran
        $filterList = Concat($filterList, $this->id_instansi->AdvancedSearch->toJson(), ","); // Field id_instansi
        $filterList = Concat($filterList, $this->dosen_pembimbing->AdvancedSearch->toJson(), ","); // Field dosen_pembimbing
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fpenempatansrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field id_penempatan
        $this->id_penempatan->AdvancedSearch->SearchValue = $filter["x_id_penempatan"] ?? "";
        $this->id_penempatan->AdvancedSearch->SearchOperator = $filter["z_id_penempatan"] ?? "";
        $this->id_penempatan->AdvancedSearch->SearchCondition = $filter["v_id_penempatan"] ?? "";
        $this->id_penempatan->AdvancedSearch->SearchValue2 = $filter["y_id_penempatan"] ?? "";
        $this->id_penempatan->AdvancedSearch->SearchOperator2 = $filter["w_id_penempatan"] ?? "";
        $this->id_penempatan->AdvancedSearch->save();

        // Field id_pendaftaran
        $this->id_pendaftaran->AdvancedSearch->SearchValue = $filter["x_id_pendaftaran"] ?? "";
        $this->id_pendaftaran->AdvancedSearch->SearchOperator = $filter["z_id_pendaftaran"] ?? "";
        $this->id_pendaftaran->AdvancedSearch->SearchCondition = $filter["v_id_pendaftaran"] ?? "";
        $this->id_pendaftaran->AdvancedSearch->SearchValue2 = $filter["y_id_pendaftaran"] ?? "";
        $this->id_pendaftaran->AdvancedSearch->SearchOperator2 = $filter["w_id_pendaftaran"] ?? "";
        $this->id_pendaftaran->AdvancedSearch->save();

        // Field id_instansi
        $this->id_instansi->AdvancedSearch->SearchValue = $filter["x_id_instansi"] ?? "";
        $this->id_instansi->AdvancedSearch->SearchOperator = $filter["z_id_instansi"] ?? "";
        $this->id_instansi->AdvancedSearch->SearchCondition = $filter["v_id_instansi"] ?? "";
        $this->id_instansi->AdvancedSearch->SearchValue2 = $filter["y_id_instansi"] ?? "";
        $this->id_instansi->AdvancedSearch->SearchOperator2 = $filter["w_id_instansi"] ?? "";
        $this->id_instansi->AdvancedSearch->save();

        // Field dosen_pembimbing
        $this->dosen_pembimbing->AdvancedSearch->SearchValue = $filter["x_dosen_pembimbing"] ?? "";
        $this->dosen_pembimbing->AdvancedSearch->SearchOperator = $filter["z_dosen_pembimbing"] ?? "";
        $this->dosen_pembimbing->AdvancedSearch->SearchCondition = $filter["v_dosen_pembimbing"] ?? "";
        $this->dosen_pembimbing->AdvancedSearch->SearchValue2 = $filter["y_dosen_pembimbing"] ?? "";
        $this->dosen_pembimbing->AdvancedSearch->SearchOperator2 = $filter["w_dosen_pembimbing"] ?? "";
        $this->dosen_pembimbing->AdvancedSearch->save();
    ...
    public function basicSearchWhere(bool $default = false): string
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->dosen_pembimbing;
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->id_penempatan); // id_penempatan
            $this->updateSort($this->id_pendaftaran); // id_pendaftaran
            $this->updateSort($this->id_instansi); // id_instansi
            $this->updateSort($this->dosen_pembimbing); // dosen_pembimbing
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->id_penempatan->setSort("");
                $this->id_pendaftaran->setSort("");
                $this->id_instansi->setSort("");
                $this->dosen_pembimbing->setSort("");
                $this->status->setSort("");
            }
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"penempatan\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"penempatan\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"penempatan\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fpenempatanlist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fpenempatanlist\" data-key=\"" . $this->keyToJson(true) .
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->id_penempatan->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"penempatan\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "id_penempatan");
            $this->createColumnOption($option, "id_pendaftaran");
            $this->createColumnOption($option, "id_instansi");
            $this->createColumnOption($option, "dosen_pembimbing");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fpenempatansrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fpenempatansrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fpenempatanlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_penempatan", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_penempatan",
...
// Load basic search values
    ...
    public function loadRowValues(array|bool|null $row = null): void
    {
        ...
        $this->id_pendaftaran->setDbValue($row['id_pendaftaran']);
        $this->id_instansi->setDbValue($row['id_instansi']);
        $this->dosen_pembimbing->setDbValue($row['dosen_pembimbing']);
    ...
    protected function newRow(): array
    {
        ...
        $row['id_pendaftaran'] = $this->id_pendaftaran->DefaultValue;
        $row['id_instansi'] = $this->id_instansi->DefaultValue;
        $row['dosen_pembimbing'] = $this->dosen_pembimbing->DefaultValue;
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->id_pendaftaran->ViewValue = FormatNumber($this->id_pendaftaran->ViewValue, $this->id_pendaftaran->formatPattern());

            // id_instansi
            $this->id_instansi->ViewValue = $this->id_instansi->CurrentValue;
            $this->id_instansi->ViewValue = FormatNumber($this->id_instansi->ViewValue, $this->id_instansi->formatPattern());

            // dosen_pembimbing
            $this->dosen_pembimbing->ViewValue = $this->dosen_pembimbing->CurrentValue;
            ...
            $this->id_penempatan->TooltipValue = "";

            // id_pendaftaran
            $this->id_pendaftaran->HrefValue = "";
            $this->id_pendaftaran->TooltipValue = "";

            // id_instansi
            $this->id_instansi->HrefValue = "";
            $this->id_instansi->TooltipValue = "";

            // dosen_pembimbing
            $this->dosen_pembimbing->HrefValue = "";
            $this->dosen_pembimbing->TooltipValue = "";
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('penempatan_searchpanel') == 'notactive' || ReadCookie('penempatan_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fpenempatansrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('penempatan_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fpenempatansrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fpenempatansrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
