 ...
 */
class StatsMonth extends DbTable implements LookupTableInterface
{
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "stats_month"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->Month->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        ...
        $this->Hits->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        if ($row) {
            if (array_key_exists('Year', $row)) {
                AddFilter($where, QuotedName('Year', $this->Dbid) . '=' . QuotedValue($row['Year'], $this->Year->DataType, $this->Dbid));
            }
            if (array_key_exists('Month', $row)) {
                AddFilter($where, QuotedName('Month', $this->Dbid) . '=' . QuotedValue($row['Month'], $this->Month->DataType, $this->Dbid));
            }
        }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            ...
            $val = !IsEmpty($this->Year->OldValue) && !$current ? $this->Year->OldValue : $this->Year->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@Year@", AdjustSql($val), $keyFilter); // Replace key value
        }
        if (is_array($row)) {
            $val = array_key_exists('Month', $row) ? $row['Month'] : null;
        } else {
            $val = !IsEmpty($this->Month->OldValue) && !$current ? $this->Month->OldValue : $this->Month->CurrentValue;
        }
        if (!is_numeric($val)) {
            return "0=1"; // Invalid key
        }
        if ($val === null) {
            ...
            $keyFilter = str_replace("@Month@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("statsmonthlist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "statsmonthview" => $this->language->phrase("View"),
            "statsmonthedit" => $this->language->phrase("Edit"),
            "statsmonthadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "StatsMonthView",
            Config("API_ADD_ACTION") => "StatsMonthAdd",
            Config("API_EDIT_ACTION") => "StatsMonthEdit",
            Config("API_DELETE_ACTION") => "StatsMonthDelete",
            Config("API_LIST_ACTION") => "StatsMonthList",
            default => ""
        };
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("statsmonthdelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"Year\":" . VarToJson($this->Year->CurrentValue, "number");
        $json .= ",\"Month\":" . VarToJson($this->Month->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
