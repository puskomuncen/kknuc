 ...
 */
class Kegiatan extends DbTable implements LookupTableInterface
{
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "kegiatan"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->id_kegiatan->IsAutoIncrement = true; // Autoincrement field
        $this->id_kegiatan->IsPrimaryKey = true; // Primary key field
        $this->id_kegiatan->Nullable = false; // NOT NULL field
        $this->id_kegiatan->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->id_kegiatan->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        $this->nama_kegiatan->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        ...
        $this->tahun->Raw = true;
        $this->tahun->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        ...
        $this->tanggal_mulai->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        ...
        $this->tanggal_selesai->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            $this->id_kegiatan->setDbValue($conn->lastInsertId());
            $row['id_kegiatan'] = $this->id_kegiatan->DbValue;
        }
        return $result;
    }
    ...
    public function update(array $row, string|array $where = "", ?array $old = null, bool $currentFilter = true): int|bool
    {
        ...
        if ($success) {
            if (!isset($row['id_kegiatan']) && !IsEmpty($this->id_kegiatan->CurrentValue)) {
                $row['id_kegiatan'] = $this->id_kegiatan->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('id_kegiatan', $row)) {
                AddFilter($where, QuotedName('id_kegiatan', $this->Dbid) . '=' . QuotedValue($row['id_kegiatan'], $this->id_kegiatan->DataType, $this->Dbid));
            }
        }
    ...
    {
        return "`id_kegiatan` = @id_kegiatan@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->id_kegiatan->CurrentValue : $this->id_kegiatan->OldValue;
        if (IsEmpty($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey(string $key, bool $current = false, ?string $keySeparator = null): void
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->id_kegiatan->CurrentValue = $keys[0];
            } else {
                $this->id_kegiatan->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_kegiatan', $row) ? $row['id_kegiatan'] : null;
        } else {
            $val = !IsEmpty($this->id_kegiatan->OldValue) && !$current ? $this->id_kegiatan->OldValue : $this->id_kegiatan->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_kegiatan@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("kegiatanlist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "kegiatanview" => $this->language->phrase("View"),
            "kegiatanedit" => $this->language->phrase("Edit"),
            "kegiatanadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "KegiatanView",
            Config("API_ADD_ACTION") => "KegiatanAdd",
            Config("API_EDIT_ACTION") => "KegiatanEdit",
            Config("API_DELETE_ACTION") => "KegiatanDelete",
            Config("API_LIST_ACTION") => "KegiatanList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("kegiatanview", $parm);
        } else {
            $url = $this->keyUrl("kegiatanview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("kegiatanedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("kegiatanlist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("kegiatanadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("kegiatanlist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("kegiatandelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"id_kegiatan\":" . VarToJson($this->id_kegiatan->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_kegiatan->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_kegiatan->CurrentValue);
        } else {
    ...
    public function getRecordKeys(): array
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("id_kegiatan") ?? Route("id_kegiatan")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys(bool $setCurrent = true): string
    {
        ...
        foreach ($arKeys as $key) {
            if ($setCurrent) {
                $this->id_kegiatan->CurrentValue = $key;
            } else {
                $this->id_kegiatan->OldValue = $key;
            }
