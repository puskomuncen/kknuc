 ...
 * Page class
 */
class PendaftaranEdit extends Pendaftaran
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "PendaftaranEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "pendaftaranedit";
...
// Is modal
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_pendaftaran") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_pendaftaran->setQueryStringValue($keyValue);
                $this->id_pendaftaran->setOldValue($this->id_pendaftaran->QueryStringValue);
            } elseif (Post("id_pendaftaran") !== null) {
                $this->id_pendaftaran->setFormValue(Post("id_pendaftaran"));
                $this->id_pendaftaran->setOldValue($this->id_pendaftaran->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("id_pendaftaran") ?? Route("id_pendaftaran")) !== null) {
                    $this->id_pendaftaran->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->id_pendaftaran->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("pendaftaranlist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "pendaftaranlist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "pendaftaranlist") {
                            ...
                            $returnUrl = "pendaftaranlist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        if (!$this->id_pendaftaran->IsDetailKey) {
            $this->id_pendaftaran->setFormValue($val);
        }
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // id_kegiatan
        $this->id_kegiatan->setDbValueDef($newRow, $this->id_kegiatan->CurrentValue, $this->id_kegiatan->ReadOnly);
    ...
    protected function restoreEditFormFromRow(array $row): void
    {
        ...
        if (isset($row['id_kegiatan'])) { // id_kegiatan
            $this->id_kegiatan->CurrentValue = $row['id_kegiatan'];
        }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("pendaftaranlist"), "", $this->TableVar, true);
        $pageId = "edit";
