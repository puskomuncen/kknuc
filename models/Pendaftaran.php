 ...
 */
class Pendaftaran extends DbTable implements LookupTableInterface
{
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "pendaftaran"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->id_pendaftaran->IsAutoIncrement = true; // Autoincrement field
        $this->id_pendaftaran->IsPrimaryKey = true; // Primary key field
        $this->id_pendaftaran->Nullable = false; // NOT NULL field
        $this->id_pendaftaran->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->id_pendaftaran->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        // id_kegiatan
        $this->id_kegiatan = new DbField(
            $this, // Table
            'x_id_kegiatan', // Variable name
            'id_kegiatan', // Name
            '`id_kegiatan`', // Expression
            '`id_kegiatan`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`id_kegiatan`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "pendaftaran";
    }
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            $this->id_pendaftaran->setDbValue($conn->lastInsertId());
            $row['id_pendaftaran'] = $this->id_pendaftaran->DbValue;
        }
        return $result;
    }
    ...
    public function update(array $row, string|array $where = "", ?array $old = null, bool $currentFilter = true): int|bool
    {
        ...
        if ($success) {
            if (!isset($row['id_pendaftaran']) && !IsEmpty($this->id_pendaftaran->CurrentValue)) {
                $row['id_pendaftaran'] = $this->id_pendaftaran->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('id_pendaftaran', $row)) {
                AddFilter($where, QuotedName('id_pendaftaran', $this->Dbid) . '=' . QuotedValue($row['id_pendaftaran'], $this->id_pendaftaran->DataType, $this->Dbid));
            }
        }
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->id_kegiatan->DbValue = $row['id_kegiatan'];
    ...
    {
        return "`id_pendaftaran` = @id_pendaftaran@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->id_pendaftaran->CurrentValue : $this->id_pendaftaran->OldValue;
        if (IsEmpty($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey(string $key, bool $current = false, ?string $keySeparator = null): void
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->id_pendaftaran->CurrentValue = $keys[0];
            } else {
                $this->id_pendaftaran->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_pendaftaran', $row) ? $row['id_pendaftaran'] : null;
        } else {
            $val = !IsEmpty($this->id_pendaftaran->OldValue) && !$current ? $this->id_pendaftaran->OldValue : $this->id_pendaftaran->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_pendaftaran@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("pendaftaranlist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "pendaftaranview" => $this->language->phrase("View"),
            "pendaftaranedit" => $this->language->phrase("Edit"),
            "pendaftaranadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "PendaftaranView",
            Config("API_ADD_ACTION") => "PendaftaranAdd",
            Config("API_EDIT_ACTION") => "PendaftaranEdit",
            Config("API_DELETE_ACTION") => "PendaftaranDelete",
            Config("API_LIST_ACTION") => "PendaftaranList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("pendaftaranview", $parm);
        } else {
            $url = $this->keyUrl("pendaftaranview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("pendaftaranedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("pendaftaranlist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("pendaftaranadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("pendaftaranlist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("pendaftarandelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"id_pendaftaran\":" . VarToJson($this->id_pendaftaran->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_pendaftaran->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_pendaftaran->CurrentValue);
        } else {
    ...
    public function getRecordKeys(): array
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("id_pendaftaran") ?? Route("id_pendaftaran")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys(bool $setCurrent = true): string
    {
        ...
        foreach ($arKeys as $key) {
            if ($setCurrent) {
                $this->id_pendaftaran->CurrentValue = $key;
            } else {
                $this->id_pendaftaran->OldValue = $key;
            }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->id_kegiatan);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->id_kegiatan);
