 ...
 * Page class
 */
class LogbookAdd extends Logbook
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "LogbookAdd";
    ...
    // CSS class/style
    public string $CurrentPageName = "logbookadd";
...
// Is modal
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("id_logbook") ?? Route("id_logbook")) !== null) {
                $this->id_logbook->setQueryStringValue($keyValue);
            }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$oldRow) { // Record not loaded
                    ...
                    $this->terminate("logbooklist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                if ($this->addRow($oldRow)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "logbooklist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "logbookview") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "logbooklist") {
                            ...
                            $returnUrl = "logbooklist"; // Return list page content
                        }
                    }
...
// Get upload files
    ...
    {
        $this->validasi_dosen->DefaultValue = $this->validasi_dosen->getDefault(); // PHP
        $this->validasi_dosen->OldValue = $this->validasi_dosen->DefaultValue;
    }
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'kegiatan' before field var 'x_kegiatan'
        $val = $this->getFormValue("kegiatan", null) ?? $this->getFormValue("x_kegiatan", null);
        ...
        // Check field name 'validasi_dosen' before field var 'x_validasi_dosen'
        $val = $this->getFormValue("validasi_dosen", null) ?? $this->getFormValue("x_validasi_dosen", null);
        if (!$this->validasi_dosen->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->validasi_dosen->Visible = false; // Disable update for API request
            } else {
                $this->validasi_dosen->setFormValue($val);
            }
        }

        // Check field name 'id_logbook' first before field var 'x_id_logbook'
        $val = $this->hasFormValue("id_logbook") ? $this->getFormValue("id_logbook") : $this->getFormValue("x_id_logbook");
    }
    ...
    public function restoreFormValues(): void
    {
        ...
        $this->validasi_dosen->CurrentValue = $this->validasi_dosen->FormValue;
    }
    ...
    public function renderRow(): void
    {
        ...
        // id_logbook
        $this->id_logbook->RowCssClass = "row";
        ...
        // kegiatan
        $this->kegiatan->RowCssClass = "row";

        // validasi_dosen
        $this->validasi_dosen->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // kegiatan
            $this->kegiatan->setupEditAttributes();
            $this->kegiatan->EditValue = !$this->kegiatan->Raw ? HtmlDecode($this->kegiatan->CurrentValue) : $this->kegiatan->CurrentValue;
            $this->kegiatan->PlaceHolder = RemoveHtml($this->kegiatan->caption());

            // validasi_dosen
            $this->validasi_dosen->EditValue = $this->validasi_dosen->options(false);
            $this->validasi_dosen->PlaceHolder = RemoveHtml($this->validasi_dosen->caption());
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            ...
            if ($this->kegiatan->Visible && $this->kegiatan->Required) {
                if (!$this->kegiatan->IsDetailKey && IsEmpty($this->kegiatan->FormValue)) {
                    $this->kegiatan->addErrorMessage(str_replace("%s", $this->kegiatan->caption(), $this->kegiatan->RequiredErrorMessage));
                }
            }
            if ($this->validasi_dosen->Visible && $this->validasi_dosen->Required) {
                if ($this->validasi_dosen->FormValue == "") {
                    $this->validasi_dosen->addErrorMessage(str_replace("%s", $this->validasi_dosen->caption(), $this->validasi_dosen->RequiredErrorMessage));
                }
            }
    ...
    protected function getAddRow(): array
    {
        ...
        // kegiatan
        $this->kegiatan->setDbValueDef($newRow, $this->kegiatan->CurrentValue, false);

        // validasi_dosen
        $tmpBool = $this->validasi_dosen->CurrentValue;
        if ($tmpBool != "1" && $tmpBool != "0") {
            $tmpBool = !empty($tmpBool) ? "1" : "0";
        }
        $this->validasi_dosen->setDbValueDef($newRow, $tmpBool, strval($this->validasi_dosen->CurrentValue) == "");
        return $newRow;
    }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("logbooklist"), "", $this->TableVar, true);
