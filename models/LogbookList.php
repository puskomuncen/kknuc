 ...
 * Page class
 */
class LogbookList extends Logbook
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "LogbookList";
    ...
    public string $FormName = "flogbooklist";

    // CSS class/style
    public string $CurrentPageName = "logbooklist";
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Table object (logbook)
        if (!isset($GLOBALS["logbook"]) || $GLOBALS["logbook"]::class == PROJECT_NAMESPACE . "logbook") {
            $GLOBALS["logbook"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "logbookadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "logbookdelete";
        $this->MultiUpdateUrl = "logbookupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'logbook');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "logbookview"); // If View page, no primary button
                } else { // List page
...
// Is modal
        ...
        // Set up lookup cache
        $this->setupLookupOptions($this->validasi_dosen);

        // Update form name to avoid conflict
        if ($this->IsModal) {
            $this->FormName = "flogbookgrid";
        }
		...
		if (ReadCookie('logbook_searchpanel') == 'notactive' || ReadCookie('logbook_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('logbook_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("flogbooksrch");
        }
        $filterList = Concat($filterList, $this->id_logbook->AdvancedSearch->toJson(), ","); // Field id_logbook
        ...
        $filterList = Concat($filterList, $this->validasi_dosen->AdvancedSearch->toJson(), ","); // Field validasi_dosen
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("flogbooksrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field id_logbook
        $this->id_logbook->AdvancedSearch->SearchValue = $filter["x_id_logbook"] ?? "";
        $this->id_logbook->AdvancedSearch->SearchOperator = $filter["z_id_logbook"] ?? "";
        $this->id_logbook->AdvancedSearch->SearchCondition = $filter["v_id_logbook"] ?? "";
        $this->id_logbook->AdvancedSearch->SearchValue2 = $filter["y_id_logbook"] ?? "";
        $this->id_logbook->AdvancedSearch->SearchOperator2 = $filter["w_id_logbook"] ?? "";
        $this->id_logbook->AdvancedSearch->save();
        ...
        // Field validasi_dosen
        $this->validasi_dosen->AdvancedSearch->SearchValue = $filter["x_validasi_dosen"] ?? "";
        $this->validasi_dosen->AdvancedSearch->SearchOperator = $filter["z_validasi_dosen"] ?? "";
        $this->validasi_dosen->AdvancedSearch->SearchCondition = $filter["v_validasi_dosen"] ?? "";
        $this->validasi_dosen->AdvancedSearch->SearchValue2 = $filter["y_validasi_dosen"] ?? "";
        $this->validasi_dosen->AdvancedSearch->SearchOperator2 = $filter["w_validasi_dosen"] ?? "";
        $this->validasi_dosen->AdvancedSearch->save();
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->id_logbook); // id_logbook
            ...
            $this->updateSort($this->validasi_dosen); // validasi_dosen
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"logbook\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"logbook\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"logbook\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"flogbooklist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"flogbooklist\" data-key=\"" . $this->keyToJson(true) .
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->id_logbook->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"logbook\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "id_logbook");
            $this->createColumnOption($option, "id_penempatan");
            $this->createColumnOption($option, "tanggal");
            $this->createColumnOption($option, "kegiatan");
            $this->createColumnOption($option, "validasi_dosen");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"flogbooksrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"flogbooksrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="flogbooklist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_logbook", "data-rowtype" => RowType::ADD]);
...
// Load basic search values
    ...
    {
        $row = [];
        $row['id_logbook'] = $this->id_logbook->DefaultValue;
        $row['id_penempatan'] = $this->id_penempatan->DefaultValue;
        $row['tanggal'] = $this->tanggal->DefaultValue;
        $row['kegiatan'] = $this->kegiatan->DefaultValue;
        $row['validasi_dosen'] = $this->validasi_dosen->DefaultValue;
        return $row;
    }
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            // id_logbook
            $this->id_logbook->ViewValue = $this->id_logbook->CurrentValue;
            ...
            // validasi_dosen
            if (ConvertToBool($this->validasi_dosen->CurrentValue)) {
                $this->validasi_dosen->ViewValue = $this->validasi_dosen->tagCaption(1) != "" ? $this->validasi_dosen->tagCaption(1) : "Yes";
            } else {
                $this->validasi_dosen->ViewValue = $this->validasi_dosen->tagCaption(2) != "" ? $this->validasi_dosen->tagCaption(2) : "No";
            }

            // id_logbook
            $this->id_logbook->HrefValue = "";
            $this->id_logbook->TooltipValue = "";
            ...
            // validasi_dosen
            $this->validasi_dosen->HrefValue = "";
            $this->validasi_dosen->TooltipValue = "";
        }
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('logbook_searchpanel') == 'notactive' || ReadCookie('logbook_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"flogbooksrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('logbook_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"flogbooksrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"flogbooksrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
