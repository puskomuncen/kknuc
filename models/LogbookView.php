 ...
 * Page class
 */
class LogbookView extends Logbook
{
    use MessagesTrait;
    ...
    // Page object name
    public string $PageObjName = "LogbookView";
    ...
    // CSS class/style
    public string $CurrentPageName = "logbookview";
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        if (($keyValue = Get("id_logbook") ?? Route("id_logbook")) !== null) {
            $this->RecKey["id_logbook"] = $keyValue;
        }
...
// Is modal
        ...
        if (($keyValue = Get("id_logbook") ?? Route("id_logbook")) !== null) {
            ...
            $this->RecKey["id_logbook"] = $this->id_logbook->QueryStringValue;
        } elseif (Post("id_logbook") !== null) {
            $this->id_logbook->setFormValue(Post("id_logbook"));
            $this->RecKey["id_logbook"] = $this->id_logbook->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->id_logbook->setQueryStringValue($keyValue);
            $this->RecKey["id_logbook"] = $this->id_logbook->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "logbooklist"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("logbooklist"); // Return to list page
                        return;
                        ...
                        while ($this->fetch()) {
                            if (SameString($this->id_logbook->CurrentValue, $this->CurrentRow['id_logbook'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "logbooklist"; // No matching record, return to list
                    } else {
