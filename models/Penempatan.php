 ...
 */
class Penempatan extends DbTable implements LookupTableInterface
{
    ...
    // Fields
    public DbField $id_penempatan;
    public DbField $id_pendaftaran;
    public DbField $id_instansi;
    public DbField $dosen_pembimbing;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "penempatan"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id_penempatan
        $this->id_penempatan = new DbField(
            $this, // Table
            'x_id_penempatan', // Variable name
            'id_penempatan', // Name
            '`id_penempatan`', // Expression
            '`id_penempatan`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`id_penempatan`', // Virtual expression
            false, // Is virtual
        ...
        $this->id_penempatan->InputTextType = "text";
        $this->id_penempatan->Raw = true;
        $this->id_penempatan->IsAutoIncrement = true; // Autoincrement field
        $this->id_penempatan->IsPrimaryKey = true; // Primary key field
        $this->id_penempatan->Nullable = false; // NOT NULL field
        $this->id_penempatan->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->id_penempatan->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['id_penempatan'] = &$this->id_penempatan;

        // id_pendaftaran
        $this->id_pendaftaran = new DbField(
            $this, // Table
            'x_id_pendaftaran', // Variable name
            'id_pendaftaran', // Name
            '`id_pendaftaran`', // Expression
            '`id_pendaftaran`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`id_pendaftaran`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->id_pendaftaran->InputTextType = "text";
        $this->id_pendaftaran->Raw = true;
        $this->id_pendaftaran->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->id_pendaftaran->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['id_pendaftaran'] = &$this->id_pendaftaran;

        // id_instansi
        $this->id_instansi = new DbField(
            $this, // Table
            'x_id_instansi', // Variable name
            'id_instansi', // Name
            '`id_instansi`', // Expression
            '`id_instansi`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`id_instansi`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->id_instansi->InputTextType = "text";
        $this->id_instansi->Raw = true;
        $this->id_instansi->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->id_instansi->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['id_instansi'] = &$this->id_instansi;

        // dosen_pembimbing
        $this->dosen_pembimbing = new DbField(
            $this, // Table
            'x_dosen_pembimbing', // Variable name
            'dosen_pembimbing', // Name
            '`dosen_pembimbing`', // Expression
            '`dosen_pembimbing`', // Basic search expression
            200, // Type
            100, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`dosen_pembimbing`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->dosen_pembimbing->InputTextType = "text";
        $this->dosen_pembimbing->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['dosen_pembimbing'] = &$this->dosen_pembimbing;
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            $this->id_penempatan->setDbValue($conn->lastInsertId());
            $row['id_penempatan'] = $this->id_penempatan->DbValue;
        }
        return $result;
    }
    ...
    public function update(array $row, string|array $where = "", ?array $old = null, bool $currentFilter = true): int|bool
    {
        ...
        if ($success) {
            if (!isset($row['id_penempatan']) && !IsEmpty($this->id_penempatan->CurrentValue)) {
                $row['id_penempatan'] = $this->id_penempatan->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('id_penempatan', $row)) {
                AddFilter($where, QuotedName('id_penempatan', $this->Dbid) . '=' . QuotedValue($row['id_penempatan'], $this->id_penempatan->DataType, $this->Dbid));
            }
        }
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->id_pendaftaran->DbValue = $row['id_pendaftaran'];
        $this->id_instansi->DbValue = $row['id_instansi'];
        $this->dosen_pembimbing->DbValue = $row['dosen_pembimbing'];
    ...
    {
        return "`id_penempatan` = @id_penempatan@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->id_penempatan->CurrentValue : $this->id_penempatan->OldValue;
        if (IsEmpty($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey(string $key, bool $current = false, ?string $keySeparator = null): void
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->id_penempatan->CurrentValue = $keys[0];
            } else {
                $this->id_penempatan->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_penempatan', $row) ? $row['id_penempatan'] : null;
        } else {
            $val = !IsEmpty($this->id_penempatan->OldValue) && !$current ? $this->id_penempatan->OldValue : $this->id_penempatan->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_penempatan@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("penempatanlist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "penempatanview" => $this->language->phrase("View"),
            "penempatanedit" => $this->language->phrase("Edit"),
            "penempatanadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "PenempatanView",
            Config("API_ADD_ACTION") => "PenempatanAdd",
            Config("API_EDIT_ACTION") => "PenempatanEdit",
            Config("API_DELETE_ACTION") => "PenempatanDelete",
            Config("API_LIST_ACTION") => "PenempatanList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("penempatanview", $parm);
        } else {
            $url = $this->keyUrl("penempatanview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("penempatanedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("penempatanlist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("penempatanadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("penempatanlist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("penempatandelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"id_penempatan\":" . VarToJson($this->id_penempatan->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_penempatan->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_penempatan->CurrentValue);
        } else {
    ...
    public function getRecordKeys(): array
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("id_penempatan") ?? Route("id_penempatan")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys(bool $setCurrent = true): string
    {
        ...
        foreach ($arKeys as $key) {
            if ($setCurrent) {
                $this->id_penempatan->CurrentValue = $key;
            } else {
                $this->id_penempatan->OldValue = $key;
            }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->id_penempatan);
                    $doc->exportCaption($this->id_pendaftaran);
                    $doc->exportCaption($this->id_instansi);
                    $doc->exportCaption($this->dosen_pembimbing);
        ...
        while (($row = $result->fetchAssociative()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->id_penempatan);
                        $doc->exportField($this->id_pendaftaran);
                        $doc->exportField($this->id_instansi);
                        $doc->exportField($this->dosen_pembimbing);
