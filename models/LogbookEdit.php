 ...
 * Page class
 */
class LogbookEdit extends Logbook
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "LogbookEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "logbookedit";
...
// Is modal
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_logbook") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_logbook->setQueryStringValue($keyValue);
                $this->id_logbook->setOldValue($this->id_logbook->QueryStringValue);
            } elseif (Post("id_logbook") !== null) {
                $this->id_logbook->setFormValue(Post("id_logbook"));
                $this->id_logbook->setOldValue($this->id_logbook->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("id_logbook") ?? Route("id_logbook")) !== null) {
                    ...
                    $this->id_logbook->CurrentValue = null;
                }
            }
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'id_logbook' before field var 'x_id_logbook'
        $val = $this->getFormValue("id_logbook", null) ?? $this->getFormValue("x_id_logbook", null);
        if (!$this->id_logbook->IsDetailKey) {
            $this->id_logbook->setFormValue($val);
        }
    ...
    {
        $this->id_logbook->CurrentValue = $this->id_logbook->FormValue;
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // id_logbook
            $this->id_logbook->setupEditAttributes();
            $this->id_logbook->EditValue = $this->id_logbook->CurrentValue;
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            if ($this->id_logbook->Visible && $this->id_logbook->Required) {
                if (!$this->id_logbook->IsDetailKey && IsEmpty($this->id_logbook->FormValue)) {
                    $this->id_logbook->addErrorMessage(str_replace("%s", $this->id_logbook->caption(), $this->id_logbook->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // kegiatan
        $this->kegiatan->setDbValueDef($newRow, $this->kegiatan->CurrentValue, $this->kegiatan->ReadOnly);
        ...
        $this->validasi_dosen->setDbValueDef($newRow, $tmpBool, $this->validasi_dosen->ReadOnly);
        return $newRow;
    }
    ...
    protected function restoreEditFormFromRow(array $row): void
    {
        ...
        if (isset($row['kegiatan'])) { // kegiatan
            $this->kegiatan->CurrentValue = $row['kegiatan'];
        }
        if (isset($row['validasi_dosen'])) { // validasi_dosen
            $this->validasi_dosen->CurrentValue = $row['validasi_dosen'];
        }
    }
