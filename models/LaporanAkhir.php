...
/**
 * Table class for laporan_akhir
 */
class LaporanAkhir extends DbTable implements LookupTableInterface
{
    ...
    // Fields
    public DbField $id_laporan;
    public DbField $id_penempatan;
    public DbField $file_laporan;
    public DbField $nilai_dosen;
    public DbField $nilai_instansi;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "laporan_akhir"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id_laporan
        $this->id_laporan = new DbField(
            $this, // Table
            'x_id_laporan', // Variable name
            'id_laporan', // Name
            '`id_laporan`', // Expression
            '`id_laporan`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`id_laporan`', // Virtual expression
            false, // Is virtual
        ...
        $this->id_laporan->InputTextType = "text";
        $this->id_laporan->Raw = true;
        $this->id_laporan->IsAutoIncrement = true; // Autoincrement field
        $this->id_laporan->IsPrimaryKey = true; // Primary key field
        $this->id_laporan->Nullable = false; // NOT NULL field
        $this->id_laporan->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->id_laporan->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['id_laporan'] = &$this->id_laporan;
        ...
        // file_laporan
        $this->file_laporan = new DbField(
            $this, // Table
            'x_file_laporan', // Variable name
            'file_laporan', // Name
            '`file_laporan`', // Expression
            '`file_laporan`', // Basic search expression
            200, // Type
            255, // Size
            -1, // Date/Time format
            false, // Is upload field
        ...
        // nilai_dosen
        $this->nilai_dosen = new DbField(
            $this, // Table
            'x_nilai_dosen', // Variable name
            'nilai_dosen', // Name
            '`nilai_dosen`', // Expression
            '`nilai_dosen`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`nilai_dosen`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->nilai_dosen->InputTextType = "text";
        $this->nilai_dosen->Raw = true;
        $this->nilai_dosen->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->nilai_dosen->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['nilai_dosen'] = &$this->nilai_dosen;

        // nilai_instansi
        $this->nilai_instansi = new DbField(
            $this, // Table
            'x_nilai_instansi', // Variable name
            'nilai_instansi', // Name
            '`nilai_instansi`', // Expression
            '`nilai_instansi`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`nilai_instansi`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->nilai_instansi->InputTextType = "text";
        $this->nilai_instansi->Raw = true;
        $this->nilai_instansi->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->nilai_instansi->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['nilai_instansi'] = &$this->nilai_instansi;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "laporan_akhir";
    }
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            $this->id_laporan->setDbValue($conn->lastInsertId());
            $row['id_laporan'] = $this->id_laporan->DbValue;
        }
        return $result;
    }
    ...
    public function update(array $row, string|array $where = "", ?array $old = null, bool $currentFilter = true): int|bool
    {
        ...
        if ($success) {
            if (!isset($row['id_laporan']) && !IsEmpty($this->id_laporan->CurrentValue)) {
                $row['id_laporan'] = $this->id_laporan->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('id_laporan', $row)) {
                AddFilter($where, QuotedName('id_laporan', $this->Dbid) . '=' . QuotedValue($row['id_laporan'], $this->id_laporan->DataType, $this->Dbid));
            }
        }
    ...
    {
        $keys = [];
        $val = $current ? $this->id_laporan->CurrentValue : $this->id_laporan->OldValue;
        if (IsEmpty($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey(string $key, bool $current = false, ?string $keySeparator = null): void
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->id_laporan->CurrentValue = $keys[0];
            } else {
                $this->id_laporan->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_laporan', $row) ? $row['id_laporan'] : null;
        } else {
            $val = !IsEmpty($this->id_laporan->OldValue) && !$current ? $this->id_laporan->OldValue : $this->id_laporan->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_laporan@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("laporanakhirlist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "laporanakhirview" => $this->language->phrase("View"),
            "laporanakhiredit" => $this->language->phrase("Edit"),
            "laporanakhiradd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "LaporanAkhirView",
            Config("API_ADD_ACTION") => "LaporanAkhirAdd",
            Config("API_EDIT_ACTION") => "LaporanAkhirEdit",
            Config("API_DELETE_ACTION") => "LaporanAkhirDelete",
            Config("API_LIST_ACTION") => "LaporanAkhirList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("laporanakhirview", $parm);
        } else {
            $url = $this->keyUrl("laporanakhirview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("laporanakhiredit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("laporanakhirlist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("laporanakhiradd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("laporanakhirlist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("laporanakhirdelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"id_laporan\":" . VarToJson($this->id_laporan->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_laporan->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_laporan->CurrentValue);
        } else {
    ...
    public function getRecordKeys(): array
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("id_laporan") ?? Route("id_laporan")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys(bool $setCurrent = true): string
    {
        ...
        foreach ($arKeys as $key) {
            if ($setCurrent) {
                $this->id_laporan->CurrentValue = $key;
            } else {
                $this->id_laporan->OldValue = $key;
            }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->id_laporan);
                    $doc->exportCaption($this->id_penempatan);
                    $doc->exportCaption($this->file_laporan);
                    $doc->exportCaption($this->nilai_dosen);
                    $doc->exportCaption($this->nilai_instansi);
                } else {
                    $doc->exportCaption($this->id_laporan);
                    $doc->exportCaption($this->id_penempatan);
                    $doc->exportCaption($this->file_laporan);
        ...
        while (($row = $result->fetchAssociative()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->id_laporan);
                        $doc->exportField($this->id_penempatan);
                        $doc->exportField($this->file_laporan);
                        $doc->exportField($this->nilai_dosen);
                        $doc->exportField($this->nilai_instansi);
                    } else {
                        $doc->exportField($this->id_laporan);
                        $doc->exportField($this->id_penempatan);
                        $doc->exportField($this->file_laporan);
