...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'prodi' before field var 'x_prodi'
        $val = $this->getFormValue("prodi", null) ?? $this->getFormValue("x_prodi", null);
        if (!$this->prodi->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->prodi->Visible = false; // Disable update for API request
            } else {
                $this->prodi->setFormValue($val);
            }
        }

        // Check field name 'fakultas' before field var 'x_fakultas'
        $val = $this->getFormValue("fakultas", null) ?? $this->getFormValue("x_fakultas", null);
    ...
    public function restoreFormValues(): void
    {
        ...
        $this->prodi->CurrentValue = $this->prodi->FormValue;
    ...
    public function loadRowValues(array|bool|null $row = null): void
    {
        ...
        $this->prodi->setDbValue($row['prodi']);
    ...
    protected function newRow(): array
    {
        ...
        $row['prodi'] = $this->prodi->DefaultValue;
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // prodi
            $this->prodi->setupEditAttributes();
            $this->prodi->EditValue = !$this->prodi->Raw ? HtmlDecode($this->prodi->CurrentValue) : $this->prodi->CurrentValue;
            $this->prodi->PlaceHolder = RemoveHtml($this->prodi->caption());

            // fakultas
            $this->fakultas->setupEditAttributes();
            $this->fakultas->EditValue = !$this->fakultas->Raw ? HtmlDecode($this->fakultas->CurrentValue) : $this->fakultas->CurrentValue;
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            ...
            if ($this->prodi->Visible && $this->prodi->Required) {
                if (!$this->prodi->IsDetailKey && IsEmpty($this->prodi->FormValue)) {
                    $this->prodi->addErrorMessage(str_replace("%s", $this->prodi->caption(), $this->prodi->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // prodi
        $this->prodi->setDbValueDef($newRow, $this->prodi->CurrentValue, $this->prodi->ReadOnly);
        ...
        // no_hp
        $this->no_hp->setDbValueDef($newRow, $this->no_hp->CurrentValue, $this->no_hp->ReadOnly);
        return $newRow;
    }
    ...
    protected function restoreEditFormFromRow(array $row): void
    {
        ...
        if (isset($row['prodi'])) { // prodi
            $this->prodi->CurrentValue = $row['prodi'];
        }
        ...
        if (isset($row['no_hp'])) { // no_hp
            $this->no_hp->CurrentValue = $row['no_hp'];
        }
    }
