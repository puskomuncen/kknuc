...
/**
 * Table class for instansi
 */
class Instansi extends DbTable implements LookupTableInterface
{
    ...
    public DbField $nama_instansi;
    public DbField $alamat;
    public DbField $kontak_person;
    public DbField $no_hp;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "instansi"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->id_instansi->IsAutoIncrement = true; // Autoincrement field
        $this->id_instansi->IsPrimaryKey = true; // Primary key field
        $this->id_instansi->Nullable = false; // NOT NULL field
        $this->id_instansi->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->id_instansi->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['id_instansi'] = &$this->id_instansi;

        // nama_instansi
        $this->nama_instansi = new DbField(
            $this, // Table
            'x_nama_instansi', // Variable name
            'nama_instansi', // Name
            '`nama_instansi`', // Expression
            '`nama_instansi`', // Basic search expression
            200, // Type
            100, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`nama_instansi`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->nama_instansi->InputTextType = "text";
        $this->nama_instansi->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['nama_instansi'] = &$this->nama_instansi;
        ...
        // kontak_person
        $this->kontak_person = new DbField(
            $this, // Table
            'x_kontak_person', // Variable name
            'kontak_person', // Name
            '`kontak_person`', // Expression
            '`kontak_person`', // Basic search expression
            200, // Type
            100, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`kontak_person`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->kontak_person->InputTextType = "text";
        $this->kontak_person->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['kontak_person'] = &$this->kontak_person;

        // no_hp
        $this->no_hp = new DbField(
            $this, // Table
            'x_no_hp', // Variable name
            'no_hp', // Name
            '`no_hp`', // Expression
            '`no_hp`', // Basic search expression
            200, // Type
            15, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`no_hp`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->no_hp->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "instansi";
    }
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            $this->id_instansi->setDbValue($conn->lastInsertId());
            $row['id_instansi'] = $this->id_instansi->DbValue;
        }
        return $result;
    }
    ...
    public function update(array $row, string|array $where = "", ?array $old = null, bool $currentFilter = true): int|bool
    {
        ...
        if ($success) {
            if (!isset($row['id_instansi']) && !IsEmpty($this->id_instansi->CurrentValue)) {
                $row['id_instansi'] = $this->id_instansi->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('id_instansi', $row)) {
                AddFilter($where, QuotedName('id_instansi', $this->Dbid) . '=' . QuotedValue($row['id_instansi'], $this->id_instansi->DataType, $this->Dbid));
            }
        }
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->nama_instansi->DbValue = $row['nama_instansi'];
    ...
    {
        return "`id_instansi` = @id_instansi@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->id_instansi->CurrentValue : $this->id_instansi->OldValue;
        if (IsEmpty($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey(string $key, bool $current = false, ?string $keySeparator = null): void
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->id_instansi->CurrentValue = $keys[0];
            } else {
                $this->id_instansi->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_instansi', $row) ? $row['id_instansi'] : null;
        } else {
            $val = !IsEmpty($this->id_instansi->OldValue) && !$current ? $this->id_instansi->OldValue : $this->id_instansi->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_instansi@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("instansilist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "instansiview" => $this->language->phrase("View"),
            "instansiedit" => $this->language->phrase("Edit"),
            "instansiadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "InstansiView",
            Config("API_ADD_ACTION") => "InstansiAdd",
            Config("API_EDIT_ACTION") => "InstansiEdit",
            Config("API_DELETE_ACTION") => "InstansiDelete",
            Config("API_LIST_ACTION") => "InstansiList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("instansiview", $parm);
        } else {
            $url = $this->keyUrl("instansiview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("instansiedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("instansilist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("instansiadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("instansilist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("instansidelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"id_instansi\":" . VarToJson($this->id_instansi->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_instansi->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_instansi->CurrentValue);
        } else {
    ...
    public function getRecordKeys(): array
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("id_instansi") ?? Route("id_instansi")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys(bool $setCurrent = true): string
    {
        ...
        foreach ($arKeys as $key) {
            if ($setCurrent) {
                $this->id_instansi->CurrentValue = $key;
            } else {
                $this->id_instansi->OldValue = $key;
            }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->nama_instansi);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->nama_instansi);
